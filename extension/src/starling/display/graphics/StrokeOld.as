package starling.display.graphics
{
	...
	import starling.display.graphics.StrokeVertexOld;
	import starling.textures.Texture;
	import starling.display.graphics.util.TriangleUtil;
	import starling.utils.MatrixUtil;
		
	public class StrokeOld extends Graphic
	{
		protected var _line			:Vector.<StrokeVertexOld>;
		...
		public function clear():void
		{
			...
			if (_line)
			{
				StrokeVertexOld.returnInstances(_line);
				_line.length = 0;
			}
			else
				_line = new Vector.<StrokeVertexOld>;
		...
		public function addDegenerates(destX:Number, destY:Number):void
		{
			...
			var lastVertex:StrokeVertexOld = _line[_numVertices-1];
		...
		{
			var v:StrokeVertexOld = _line[index];
			v.x = x;
			v.y = y;
			if ( isInvalid == false )
				setGeometryInvalid();
	...
	//	[Deprecated(replacement="starling.display.graphics.Stroke.lineTo()")]
		...
		{							
			...
			if ( textures.length > 0 && _line.length > 0 )
			{
				...
				var prevVertex:StrokeVertexOld = _line[_line.length - 1];
			...
			var v:StrokeVertexOld = StrokeVertexOld.getInstance();
		...
		///////////////////////////////////
		[inline]
		protected static function createPolyLinePreAlloc( _line:Vector.<StrokeVertexOld>, 
												...
												_hasDegenerates:Boolean):void 
		{
			...
			for ( var i:int = 0; i < numVertices; i++ )
			{
				...
				var v0:StrokeVertexOld = _line[idx0];
				var v1:StrokeVertexOld = _line[idx];
				var v2:StrokeVertexOld = _line[idx2];
		...
		///////////////////////////////////
		[inline]
		protected static function createPolyLine( vertices:Vector.<StrokeVertexOld>, 
												...
												indexOffset:int ):void
		{
			...
			for ( var i:int = 0; i < numVertices; i++ )
			{
				...
				var v0:StrokeVertexOld = vertices[idx0];
				var v1:StrokeVertexOld = vertices[idx];
				var v2:StrokeVertexOld = vertices[idx2];
		...
		protected static function fixUpPolyLine( vertices:Vector.<StrokeVertexOld> ): int
		{
		...
		override protected function shapeHitTestLocalInternal( localX:Number, localY:Number ):Boolean
		{
			...
			{
				var v0:StrokeVertexOld = _line[i - 1];
				var v1:StrokeVertexOld = _line[i];
		...
		public static function strokeCollideTest(s1:StrokeOld, s2:StrokeOld, intersectPoint:Point, staticLenIntersectPoints:Vector.<Point> = null ) : Boolean
		{
			...
			{
				var s1v0:StrokeVertexOld = s1._line[i - 1];
				var s1v1:StrokeVertexOld = s1._line[i];
				...
				{
					var s2v0:StrokeVertexOld = s2._line[j - 1];
					var s2v1:StrokeVertexOld = s2._line[j];
